...
def classify_images(images_dir, results_dic, model):
    ...
    """
    # loop over the dictionary(results_dic) to process each pet images(filename) with the classifier function to get classifier label
    for image_file in results_dic:
        classifier_label = classifier(images_dir + image_file, model).lower().strip()
        
        # add classifier label to results_dic key
        real_label = results_dic[image_file][0]
        
        # check if pet image matches classifier label
        prediction = 1 if real_label in classifier_label else 0
        
        # add results to results_dic
